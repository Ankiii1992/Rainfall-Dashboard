...
color_map = {
    "No Rain": "#f8f8f8", "Very Light": "#e0ffe0", "Light": "#00ff01", "Moderate": "#00ffff",
    "Rather Heavy": "#ffeb3b", "Heavy": "#ff8c00", "Very Heavy": "#d50000",
    "Extremely Heavy": "#f820fe", "Exceptional": "#e8aaf5"
}
category_ranges = {
...
def classify_rainfall(rainfall):
    ...
    elif rainfall <= 2.4: return "Very Light"
    elif rainfall <= 7.5: return "Light"
    elif rainfall <= 35.5: return "Moderate"
    elif rainfall <= 64.4: return "Rather Heavy"
    elif rainfall <= 124.4: return "Heavy"
    elif rainfall <= 244.4: return "Very Heavy"
    elif rainfall <= 350: return "Extremely Heavy"
...
def plot_choropleth(df, geojson_path, title="Gujarat Rainfall Distribution", geo_feature_id_key="properties.SUB_DISTRICT", geo_location_col="Taluka"):
    ...
    color_column = 'Total_mm' if 'Total_mm' in df_plot.columns else 'District_Avg_Rain_Last_24_Hrs'
    if color_column:
    ...
    fig = px.choropleth_mapbox(
        df_plot, geojson=geojson_data, featureidkey=geo_feature_id_key,
        ...
        hover_data={
            color_column: ":.1f mm", "District": True if geo_location_col == "Taluka" else False,
            "Rainfall_Category": False
        },
        height=650, title=f"<b>{title}</b>"
    )
    fig.update_layout(
        margin={"r":0,"t":0,"l":0,"b":0}, uirevision='true', showlegend=True,
        legend=dict(
            orientation="h", yanchor="top", y=-0.15, xanchor="center", x=0.5,
            title_text="<b>Rainfall Categories (mm)</b>", font=dict(size=10), itemsizing='constant',
        ),
        annotations=[
            go.layout.Annotation(
                text="<b>Gujarat Weatherman</b>", xref="paper", yref="paper",
                x=0.99, y=0.01, showarrow=False,
                font=dict(size=14, color="rgba(0,0,0,0.3)"), xanchor='right', yanchor='bottom'
            )
        ],
...
def show_24_hourly_dashboard(df, selected_date):
    ...
    if "Rain_Last_24_Hrs" in df.columns: df.rename(columns={"Rain_Last_24_Hrs": "Total_mm"}, inplace=True)
    required_cols = ["Total_mm", "Taluka", "District"]
    for col in required_cols:
        if col not in df.columns:
            st.error(f"Required column '{col}' not found in the loaded data.")
            return
    if 'Total_Rainfall' not in df.columns: df['Total_Rainfall'] = df['Total_mm'] * 1.5
    if 'Percent_Against_Avg' not in df.columns: df['Percent_Against_Avg'] = (df['Total_Rainfall'] / 700) * 100
    ...
    with col_donut:
        ...
        donut_data = pd.DataFrame({'Category': ['Completed', 'Remaining'], 'Value': [state_rainfall_progress_percentage, 100 - state_rainfall_progress_percentage]})
        fig_donut = go.Figure(data=[go.Pie(
            labels=donut_data['Category'], values=donut_data['Value'], hole=0.7,
            marker=dict(colors=['#28a745', '#e0e0e0'], line=dict(color='white', width=4)),
            hoverinfo="none", textinfo="none"
        )])
        fig_donut.update_layout(
            height=320, margin=dict(l=20, r=20, t=50, b=20), paper_bgcolor='rgba(0,0,0,0)', showlegend=False,
            annotations=[dict(text=f"<b>{state_rainfall_progress_percentage:.1f}%</b>", x=0.5, y=0.5, font_size=40, showarrow=False, font_color='#01579b')]
        )
        st.plotly_chart(fig_donut, use_container_width=True)
    with col_metrics:
        col_top1, col_top2 = st.columns(2)
        with col_top1: st.markdown(f"<div class='metric-tile'><h4><b>State Total Seasonal Rainfall Till Today (Avg.)</b></h4><h2>{state_total_seasonal_avg:.1f} mm</h2></div>", unsafe_allow_html=True)
        with col_top2: st.markdown(f"<div class='metric-tile'><h4><b>State Avg. Rain (last 24 hrs)</b></h4><h2>{state_avg_24hr:.1f} mm</h2></div>", unsafe_allow_html=True)
        col_bottom1, col_bottom2, col_bottom3 = st.columns(3)
        with col_bottom1: st.markdown(f"<div class='metric-tile'><h4><b>Highest Rainfall District (Talukas Avg.)</b></h4><h2>{highest_district}</h2><p>({highest_district_avg:.1f} mm)</p></div>", unsafe_allow_html=True)
        with col_bottom2: st.markdown(f"<div class='metric-tile'><h4><b>Highest Rainfall Taluka</b></h4><h2>{highest_taluka['Taluka']}</h2><p>({highest_taluka['Total_mm']:.1f} mm)</p></div>", unsafe_allow_html=True)
        with col_bottom3: st.markdown(f"<div class='metric-tile'><h4><b>Talukas with Rainfall Today</b></h4><h2>{num_talukas_with_rain_today}</h2><p>({TOTAL_TALUKAS_GUJARAT} Total Talukas)</p></div>", unsafe_allow_html=True)
    ...
    district_rainfall_avg_df = district_rainfall_avg_df.rename(columns={'Total_mm': 'District_Avg_Rain_Last_24_Hrs'})
    district_rainfall_avg_df["Rainfall_Category"] = pd.Categorical(district_rainfall_avg_df["District_Avg_Rain_Last_24_Hrs"].apply(classify_rainfall), categories=ordered_categories, ordered=True)
    ...
    df_map_talukas["Rainfall_Category"] = pd.Categorical(df_map_talukas["Total_mm"].apply(classify_rainfall), categories=ordered_categories, ordered=True)
    ...
    with tab_districts:
        ...
        with map_col_dist:
            ...
            with st.spinner("Loading district map..."):
                fig_map_districts = plot_choropleth(district_rainfall_avg_df, "gujarat_district_clean.geojson", title="Gujarat Daily Rainfall Distribution by District", geo_feature_id_key="properties.district", geo_location_col="District")
        ...
        with insights_col_dist:
            ...
            fig_category_dist_dist = px.bar(
                category_counts_dist, x='Category', y='Count', title='<b>Distribution of Districts by Daily Rainfall Category</b>',
                labels={'Count': '<b>Number of Districts</b>'}, color='Category', color_discrete_map=color_map,
                hover_data={'Category': True, 'Rainfall_Range': True, 'Count': True}
            )
            fig_category_dist_dist.update_layout(
                xaxis=dict(tickmode='array', tickvals=category_counts_dist['Category'], ticktext=[f"<b>{cat}</b>" for cat in category_counts_dist['Category']], tickangle=-45),
                xaxis_title=None, yaxis_title="<b>Number of Districts</b>", showlegend=False, height=350, margin=dict(l=0, r=0, t=50, b=0)
            )
    ...
    with tab_talukas:
        ...
        with map_col_tal:
            ...
            with st.spinner("Loading taluka map..."):
                fig_map_talukas = plot_choropleth(df_map_talukas, "gujarat_taluka_clean.geojson", title="Gujarat Daily Rainfall Distribution by Taluka", geo_feature_id_key="properties.SUB_DISTRICT", geo_location_col="Taluka")
        ...
        with insights_col_tal:
            ...
            pie_data = pd.DataFrame({'Category': ['Talukas with Rainfall', 'Talukas without Rainfall'], 'Count': [num_talukas_with_rain_today, talukas_without_rain]})
            fig_pie = px.pie(pie_data, values='Count', names='Category', title="<b>Percentage of Talukas with Daily Rainfall</b>", color='Category', color_discrete_map={'Talukas with Rainfall': '#28a745', 'Talukas without Rainfall': '#dc3545'})
            ...
            fig_category_dist_tal = px.bar(
                category_counts_tal, x='Category', y='Count', title='<b>Distribution of Talukas by Daily Rainfall Category</b>',
                labels={'Count': '<b>Number of Talukas</b>'}, color='Category', color_discrete_map=color_map,
                hover_data={'Category': True, 'Rainfall_Range': True, 'Count': True}
            )
            fig_category_dist_tal.update_layout(
                xaxis=dict(tickmode='array', tickvals=category_counts_tal['Category'], ticktext=[f"<b>{cat}</b>" for cat in category_counts_tal['Category']], tickangle=-45),
                xaxis_title=None, yaxis_title="<b>Number of Talukas</b>", showlegend=False, height=350, margin=dict(l=0, r=0, t=50, b=0)
            )
    ...
    if not df_top_10.empty:
        fig_top_10 = px.bar(
            df_top_10, x='Taluka', y='Total_mm', color='Total_mm', color_continuous_scale=px.colors.sequential.Bluyl,
            labels={'Total_mm': '<b>Total Rainfall (mm)</b>', 'Taluka':'<b>Taluka</b>'}, hover_data=['District'],
            text='Total_mm', title='<b>Top 10 Talukas with Highest Total Daily Rainfall</b>'
        )
        ...
        fig_top_10.update_layout(
            xaxis_tickangle=-45, showlegend=False, margin=dict(t=50), coloraxis_showscale=False,
            yaxis_title="<b>Total Rainfall (mm)</b>", xaxis_title="<b>Taluka</b>"
        )
        fig_top_10.update_layout(modebar_remove=['toImage', 'sendDataToCloud', 'hoverClosestCartesian', 'toggleSpikelines'])
        st.plotly_chart(fig_top_10, use_container_width=True)
    else: st.info("No rainfall data available to determine top 10 talukas.")
...
with col_date_picker:
    selected_date_from_picker = st.date_input("Choose Date", value=st.session_state.selected_date, help="Select a specific date to view its rainfall summary.")
...
with tab_daily:
    ...
    else: st.warning(f"⚠️ Daily data is not available for {selected_date_str}.")
...
with tab_hourly:
    ...
    if not df_2hr.empty:
        ...
        for col in existing_order: df_2hr[col] = pd.to_numeric(df_2hr[col], errors="coerce")
        df_2hr['Total_mm'] = df_2hr[existing_order].sum(axis=1)
        df_long = df_2hr.melt(id_vars=["District", "Taluka", "Total_mm"], value_vars=existing_order, var_name="Time Slot", value_name="Rainfall (mm)")
        df_long = df_long.dropna(subset=["Rainfall (mm)"])
        df_long['Taluka'] = df_long['Taluka'].str.strip()
        df_long = df_long.groupby(["District", "Taluka", "Time Slot"], as_index=False).agg({"Rainfall (mm)": "sum", "Total_mm": "first"})
        ...
        df_long['Time Slot Label'] = pd.Categorical(df_long['Time Slot'].map(slot_labels), categories=[slot_labels[slot] for slot in existing_order], ordered=True)
        ...
        for col, (label, value) in zip(row1, row1_titles):
            with col:
                st.markdown("<div class='metric-tile'><h4><b>{label}</b></h4><h2>{value}</h2></div>", unsafe_allow_html=True)
        ...
        if selected_talukas:
            ...
            fig = px.line(plot_df, x="Time Slot Label", y="Rainfall (mm)", color="Taluka", markers=True, text="Rainfall (mm)", title="<b>Rainfall Trend Over Time for Selected Talukas</b>", labels={"Rainfall (mm)": "<b>Rainfall (mm)</b>", "Time Slot Label": "<b>Time Slot</b>"})
            fig.update_traces(texttemplate='<b>%{text:.1f}</b>', textposition="top center", hovertemplate='<b>%{text:.1f}</b>')
            fig.update_layout(showlegend=True, xaxis_title="<b>Time Slot</b>", yaxis_title="<b>Rainfall (mm)</b>")
            fig.update_layout(modebar_remove=['toImage', 'sendDataToCloud', 'hoverClosestCartesian', 'toggleSpikelines'])
            st.plotly_chart(fig, use_container_width=True)
        else: st.info("Please select at least one Taluka to view the rainfall trend.")
        
        # New advanced search and aggregation feature
        st.markdown("---")
        st.markdown("### 🔎 <b>Advanced Data Aggregation</b>", unsafe_allow_html=True)
        col_group, col_agg = st.columns(2)
        with col_group:
            group_by_columns = st.multiselect(
                "Group by:",
                options=['Zone', 'District', 'Taluka'],
                default=['District']
            )
        with col_agg:
        ...
        if group_by_columns and aggregation_functions:
            try:
                # Add 'Total_mm' column if not present to ensure aggregation works
                if 'Total_mm' not in df_2hr.columns:
                    df_2hr['Total_mm'] = df_2hr.loc[:, time_slot_columns].sum(axis=1)

                # Ensure 'Zone' is available for grouping, if needed
                if 'Zone' in group_by_columns:
                    # This is placeholder logic, replace with your actual Zone mapping
                    zone_mapping = {'District A': 'North', 'District B': 'South', 'District C': 'North'}
                    df_2hr['Zone'] = df_2hr['District'].map(zone_mapping)

                aggregated_df = df_2hr.groupby(group_by_columns)['Total_mm'].agg(aggregation_functions)
                st.dataframe(aggregated_df.reset_index(), use_container_width=True)
            except KeyError as e:
                st.error(f"Error: A column selected for grouping is not in the data. Please check your data source. Original error: {e}")
        else:
            st.info("Select columns to group by and aggregation methods to view the aggregated table.")
    else: st.warning(f"⚠️ 2-Hourly data is not available for {selected_date_str}.")
