...
def show_24_hourly_dashboard(df, selected_date, taluka_geojson, district_geojson):
    ...
    # --- START OF NEW CODE: Zonewise Summary Table and Dual-Axis Chart ---
    st.markdown("---")
    st.subheader("Zonewise Rainfall Summary")

    # The data is already loaded in `df` (which is `df_24hr` from the main flow)
    df_zonewise = df.groupby('Zone').agg(
        Avg_Rain=('Avg_Rain', 'mean'),
        Rain_Till_Yesterday=('Rain_Till_Yesterday', 'mean'),
        Rain_Last_24_Hrs=('Rain_Last_24_Hrs', 'mean'),
        Total_Rainfall=('Total_Rainfall', 'mean'),
        Percent_Against_Avg=('Percent_Against_Avg', 'mean')
    ).reset_index()

    # Calculate and append the State Average row
    state_avg = pd.DataFrame([{
        'Zone': 'State Avg.',
        'Avg_Rain': df_zonewise['Avg_Rain'].mean(),
        'Rain_Till_Yesterday': df_zonewise['Rain_Till_Yesterday'].mean(),
        'Rain_Last_24_Hrs': df_zonewise['Rain_Last_24_Hrs'].mean(),
        'Total_Rainfall': df_zonewise['Total_Rainfall'].mean(),
        'Percent_Against_Avg': df_zonewise['Percent_Against_Avg'].mean()
    }])
    df_zonewise = pd.concat([df_zonewise, state_avg], ignore_index=True).round(2)

    # Display the summary table
    st.dataframe(df_zonewise, hide_index=True, use_container_width=True)

    st.subheader("Zonewise Total Rainfall vs. Percent Against Average")

    # Filter out the 'State Avg.' row for the chart visualization
    df_chart_data = df_zonewise[df_zonewise['Zone'] != 'State Avg.']

    # Create the bar chart for Total_Rainfall
    bar_chart = alt.Chart(df_chart_data).mark_bar().encode(
        x=alt.X('Zone', sort='-y', title='Zone'),
        y=alt.Y('Total_Rainfall', title='Total Rainfall (mm)'),
        tooltip=['Zone', 'Total_Rainfall', 'Percent_Against_Avg']
    ).properties(
        title='Zonewise Rainfall Summary'
    )

    # Create the line chart for Percent_Against_Avg on the right axis
    line_chart = alt.Chart(df_chart_data).mark_line(point=True, color='red').encode(
        x='Zone',
        y=alt.Y('Percent_Against_Avg', title='Percent Against Avg (%)'),
        tooltip=['Zone', 'Total_Rainfall', 'Percent_Against_Avg']
    )

    # Combine the two charts into a dual-axis chart
...
with tab_daily:
    ...
    if not df_24hr.empty:
        ...
        st.warning(f"⚠️ Daily data is not available for {selected_date.strftime('%Y-%m-%d')}. Please select a different date.")
...
with tab_hourly:
    ...
    if not df_2hr.empty:
        ...
        st.warning(f"⚠️ 2-Hourly data is not available for {selected_date.strftime('%Y-%m-%d')}.")
